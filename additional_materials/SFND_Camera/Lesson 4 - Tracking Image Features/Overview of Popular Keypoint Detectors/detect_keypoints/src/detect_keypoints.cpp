...
void detKeypoints1()
{
    ...
    // Use the OpenCV library to add the FAST detector in addition to the already implemented Shi-Tomasi detector.
    // Compare both algorithms with regard to: (a) number of keypoints, (b) distribution of keypoints
    // over the image and (c) processing speed.

    // Step 1: Detect the keypoint using SURF Detector
    std::vector<cv::KeyPoint> fast_key_points;
    cv::Ptr<cv::FastFeatureDetector> detector = cv::FastFeatureDetector::create(10, true);
    t = (double)cv::getTickCount();
    detector->detect(img, fast_key_points);
    t = ((double)cv::getTickCount() - t) / cv::getTickFrequency();
    cout << "FAST with n= " << fast_key_points.size() << " keypoints in " << 1000 * t / 1.0 << " ms" << endl;


    // visualize results
    visImage = img.clone();
    cv::drawKeypoints(img, fast_key_points, visImage, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);
